...
PVector origin;
float rc[] = {15, 15}; // distance to travel inwards/outwards per iteration, (set two values to the same (or close) for banding patterns - 10, 10 is good)
float initr = 200; // start radius (probably just set this to either innerRing or outerRing)
float innerRing = 150; // set to 0 for lines to disappear towards centre
float outerRing = 350; // set to >width for lines to disappear off edge
int radiusMode = 4; // 0 = barrier, 1 = allow negative, 2 = soft reflection (once), 3 = hard reflection, 4 = kill line
float[] anglemult = {0.005, 0.05, 0};
float[] startangle = {0, PI/8}; // lines will start randomly between these two angles
...
void setup(){
  ...
  background(0, 0, TWO_PI * 0.2); // * 0.2 and * 0.8 are both nice
